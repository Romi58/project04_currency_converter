...
inquirer.prompt(questions).then((answers) => {
    targetCurrency = currencies.find((currency) => currency.name === answers.targetCurrency);
    const usd = parseFloat(answers.amount);
    const convertedAmount = usd * targetCurrency.rate;
    console.log(`The amount in ${targetCurrency.name} is: ${convertedAmount.toFixed(2)}`);
});
